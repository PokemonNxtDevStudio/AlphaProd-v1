...
public abstract class ItemPickerBase<T> : EditorWindow where T : class
{
    ...
    {
        searchQuery = DatabaseEditorStyles.SearchBar(searchQuery, this, isSearching);
        if (focusOnInput)
        {
    ...
    {
        EditorGUILayout.HelpBox("Fill this with some Info on what to to .\n  This is still in the works .", MessageType.Info);
    }
}